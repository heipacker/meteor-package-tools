...
Tinytest.add('McTools.formatBalance', function (test) {
    ...
    if(Meteor.isClient)
        McTools.setUnit('mc');
    test.equal(McTools.formatBalance('1000000000000000000', '0,0.0[00] unit'), "1.0 mc");

    // default to finney
    if(Meteor.isClient) {
        McTools.setUnit('finney');
        test.equal(McTools.formatBalance('1000000000000000000', '0,0.0[00] unit'), "1,000.0 finney");

        test.equal(McTools.formatBalance('100000000000000000', '0,0.0[00] UNIT'), "100.0 FINNEY");
    }

    test.equal(McTools.formatBalance('1000000000000000000', '0,0.0[00] unit', 'gsha'), "1,000,000,000.0 gsha");

    test.equal(McTools.formatBalance('112345676543212345', '0,0.0[00] UNIT', 'gsha'), "112,345,676.543 GSHA");

    test.equal(McTools.formatBalance('112345676543212345', '0,0.0[0000]', 'gsha'), "112,345,676.54321");
